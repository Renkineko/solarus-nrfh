...
local function check_torches()
	if all_torches_lit() then
		...
		map:set_light(1)
		for _, enemy in ipairs(map:get_entities("enemy_torch")) do
			enemy:set_life(0)
		end
...
function map:on_started()
	...
	for _, enemy in ipairs(map:get_entities("flying_bubble")) do
...
function close_ke_a_sensor:on_activated()
  ...
  if ke_a:is_open()
      and map:has_entities("ke_a_enemy") then
...
for _, enemy in ipairs(map:get_entities("ke_a_enemy")) do
...
for _, enemy in ipairs(map:get_entities("pike_switch")) do
...
function final_sd_a:on_activated()
	...
	for _, switch in ipairs(map:get_entities("sd_a")) do
